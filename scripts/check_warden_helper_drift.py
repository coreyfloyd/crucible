#!/usr/bin/env python3
"""Function-scoped drift check for the warden eval-harness helpers (#464).

Invocation (from repo root):
    python3 scripts/check_warden_helper_drift.py            # check the tree
    python3 scripts/check_warden_helper_drift.py --selftest # built-in logic tests

The warden behavior-eval harness copies temper's `_dispatch_paths.py` / `_runid.py`
helpers (the design's "link/derive, never copy" rule is relaxed for these tiny
path/hash/validation utilities — copying with a machine-checked no-drift invariant is
cheaper than a shared-package refactor; #424/#373 precedent). This check tolerates the
copy's *legitimate* edits (docstring/comment rewrites) yet still catches a logic fork.

It is **function-scoped**: it diffs ONLY the bodies of the functions warden actually
imports —
...
the whole file, so the warden copy may freely omit temper-only helpers
(`validate_prefix`, `sanitize_summary`) and warden CI never reddens when temper edits a
function warden never calls.
...
warden copy legitimately rewrites — the module docstring (never in the compared set: we
...
# (reference helper, warden copy, the function names warden imports)
PAIRS = [
    ("skills/temper/evals/_dispatch_paths.py",
     "skills/warden/evals/_dispatch_paths.py",
     ("resolve_dispatch_dir", "fixture_sha", "template_sha")),
    ("skills/temper/evals/_runid.py",
     "skills/warden/evals/_runid.py",
     ("validate_run_id",)),
]

# Compared functions whose docstring the warden copy legitimately rewrites; their
...
def compare_sources(ref_src: str, copy_src: str, names) -> list:
    ...
    for name in names:
        if name not in ref:
            ...
            continue
        if name not in cp:
            drift.append(f"{name}: missing from warden copy")
            continue
        ...
        for const in sorted(refd & (set(ref_assigns) | set(cp_assigns))):
            ...
            if in_ref and in_cp:
                ...
                where = "warden copy" if in_ref else "reference (temper) helper"
                drift.append(
...
def main() -> int:
    ...
    if all_drift:
        print("WARDEN HELPER DRIFT — the copied helpers diverged from temper:")
        for d in all_drift:
            print(f"  {d}")
        print("\nRe-sync the copied function bodies with "
              "skills/temper/evals/, or update this check if the divergence is "
              "intentional. See scripts/check_warden_helper_drift.py docstring.")
        return 1
    print("OK — warden's copied helpers match temper for every imported "
...
def selftest() -> int:
    ...
    case_c = clean_copy.replace(
        "sha256 of a committed warden fixture/prompt artifact",
        "sha256 of a warden eval artifact")

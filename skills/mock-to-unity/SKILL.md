...
**Related skills:** When writing C# controllers (Layers 1, 3, 4), `repo-alpha-standards` applies to all C# code. After implementation, `test-driven-development` applies for controller tests. After self-verification, `crucible:quality-gate` validates the output.
...
| Output | Location |
|--------|----------|
| USS files | `repo-alpha/Assets/_Project/Resources/UI/` |
| C# controllers | `repo-alpha/Assets/_Project/Scripts/Systems/UI/<subsystem>/` |
| Theme.uss variable additions | `repo-alpha/Assets/_Project/Resources/UI/Theme.uss` |
...
**Layer 2 — USS Styling:** Write USS selectors using Theme.uss variables. Reference the translation map for every property. Use `var()` references exclusively — no hardcoded values. For any variable flagged as "needs adding to Theme.uss" in the translation map, add it to `repo-alpha/Assets/_Project/Resources/UI/Theme.uss` in the appropriate section with a comment. Also update `~/.claude/skills/mockup-builder/references/theme-variables.md` to include the new variable.

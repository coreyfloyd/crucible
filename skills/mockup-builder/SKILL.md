---
name: mockup-builder
description: Use when creating a visual mockup, UI prototype, or HTML reference for repo-alpha UI. Triggers on "mockup", "prototype", "UI reference", "design a panel", "mock up", or any task producing a visual HTML file for later Unity UI Toolkit implementation.
---

# Mockup Builder

Create HTML mockups for repo-alpha UI that are constrained to Theme.uss variables and designed for direct translation to Unity UI Toolkit.
...
2. Read existing mockups in `docs/mockups/` for visual language reference — but note that mockups created before this skill may use hardcoded hex values and lack CSS variables. Use them only to understand repo-alpha's visual language (colors, proportions, layout patterns), NOT as CSS architecture exemplars.
...
- Push for "bold", "unexpected", or "distinctive" aesthetics — match repo-alpha's established dark sci-fi visual language
...
1. If a browser is available, open the mockup to verify rendering. Otherwise, review the HTML/CSS source for structural correctness and consistency with repo-alpha's visual language.

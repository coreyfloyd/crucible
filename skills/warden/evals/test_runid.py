#!/usr/bin/env python3
"""run-id validation re-assertion for the warden behavior-eval harness (#464).

stdlib unittest. The validator is the copied _runid.validate_run_id; this re-asserts
its I-9 behavior in warden's own gating suite (check_warden_helper_drift.py pins the
body; this pins the behavior).
"""
import pathlib
import sys
import unittest
...
from skills.warden.evals._runid import validate_run_id  # noqa: E402


class TestRunIdValidation(unittest.TestCase):
    def test_accepts_well_formed_ids(self):
        for ok in ("run1", "2026-07-19T20-45-00", "abc_DEF-123", "_x"):

...
On the **clean-pass exit only** (Exit Precedence slot #1), after consolidating Minors and before writing the verdict marker, the orchestrator computes a **Minor advisory** — a PASS-annotating signal that weighs accumulated Minors against convergence, closing the gap that a `0F/0S`-with-Minors round terminates at slot #1 before the Minor-aware stagnation judge (slot #7) ever runs. **The advisory never changes the verdict or precedence — a clean pass stays PASS (slot #1 stands) and the advisory never blocks.** It adds **zero rounds**: every input is a file already on disk.

Two rules; either firing sets the advisory (both firing → `density+trajectory`):

- **Density (count net, R-independent).** Fire `density` when `m_exit ≥ K`, with **K = 5**. `m_exit` is the clean exit round's Minor count: the deduplicated union (by title / root-cause identity) of `round-R-findings.md`'s Minor observations AND — when look-harder fired and confirmed on round R — `round-R-look-harder.md`'s Minor observations (the live spec does not fold a confirming look-harder's Minors into `round-R-findings.md`, `:1260` — so the union is required to avoid under-counting exactly when the tightened rubric finds more). Three exit cases, all covered by the conditional union: (i) **confirmed** → findings ∪ look-harder.md; (ii) **demoted** → the round is non-terminal (`:239`) and is never the exit round; (iii) **skipped** (`:227`) → no `round-R-look-harder.md`, so `m_exit` = `round-R-findings.md` Minors only. Density is the robust primary net and catches the literal `0F/0S/7M` exit regardless of any title drift.

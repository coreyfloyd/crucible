...
4. **`skills/mockup-builder/SKILL.md`** — De-repo-alpha (5 references)
   - Replace "repo-alpha UI" → "your project's UI" or equivalent
   - Replace "repo-alpha's visual language" → "the project's visual language"
   - Generalize path references

5. **`skills/mockup-builder/references/theme-variables.md`** — De-repo-alpha (1 reference)
   - Replace repo-alpha-specific path with generic placeholder

6. **`skills/mock-to-unity/SKILL.md`** — De-repo-alpha (6 references)
   - Replace `repo-alpha/Assets/` paths with generic `Assets/` or `<project>/Assets/`
   - Remove or generalize "repo-alpha-standards" reference
   - Rename "repo-alpha-Specific Rules" section

7. **`skills/debugging/implementer-prompt.md`** — De-repo-alpha (1 reference)
   - Replace "repo-alpha.Tests.EditMode" namespace example with generic

8. **`README.md`** — Updates (additive only, no existing repo-alpha references to clean)
...
- [ ] No repo-alpha references remain in mockup-builder, mock-to-unity, or debugging skills
...
- **De-repo-alpha audit**: Grep for "repo-alpha" (case-insensitive) in all skill files post-cleanup — zero hits expected (excluding docs/plans/)

...
Three new crucible skills that form a pipeline for design-to-code UI work in repo-alpha:
...
**Relationship to Anthropic originals:** These replace `frontend-design` and `playground` entirely for repo-alpha. Crucible is its own thing. Anthropic skills were used as structural inspiration only.
...
**Triggers when:** Agent is asked to create a visual mockup, prototype, or UI reference for repo-alpha.
**Skill type:** Rigid — follow exactly.

### Core Constraints

1. **All colors must use CSS custom properties** that mirror Theme.uss variables (e.g., `var(--color-bg-base)`). The skill includes the full variable mapping from `repo-alpha/Assets/_Project/Resources/UI/Theme.uss`.
...
- Does not push for "bold" or "unexpected" aesthetics — mockups match repo-alpha's established dark sci-fi visual language
...
### repo-alpha-Specific Rules (Baked In)

...
**Goal:** Add a new `adversarial-tester` skill to crucible, integrate it into the build pipeline's Phase 3, de-repo-alpha three existing skills, and update the README.
...
Task 3 (de-repo-alpha skills) ── no deps ────────┤
...
### Task 3: De-repo-alpha Skills

Remove all repo-alpha-specific references from skills that should be project-agnostic. Replace with generic equivalents.
...
1. Edit `skills/mockup-builder/SKILL.md` — 5 replacements:
   - Line 3 (frontmatter description): `"for repo-alpha UI"` -> `"for your project's UI"`
   - Line 8 (opening paragraph): `"for repo-alpha UI"` -> `"for your project's UI"`
   - Line 15 (Before Starting, step 2): `"repo-alpha's visual language"` -> `"the project's visual language"`
   - Line 79 (What This Skill Does NOT Do): `"repo-alpha's established dark sci-fi visual language"` -> `"the project's established visual language"`
   - Line 86 (After Creating the Mockup): `"repo-alpha's visual language"` -> `"the project's visual language"`

2. Edit `skills/mockup-builder/references/theme-variables.md` — 1 replacement:
   - Line 3: `"Source: \`repo-alpha/Assets/_Project/Resources/UI/Theme.uss\`"` -> `"Source: Your project's Theme.uss file (e.g., \`Assets/_Project/Resources/UI/Theme.uss\`)"`

3. Edit `skills/mock-to-unity/SKILL.md` — 6 replacements:
   - Line 12: `"repo-alpha-standards"` -> `"your project's coding standards"` (full sentence: "When writing C# controllers (Layers 1, 3, 4), your project's coding standards apply to all C# code.")
   - Line 24: `"repo-alpha/Assets/_Project/Resources/UI/"` -> `"Assets/_Project/Resources/UI/"` (or `"<project>/Assets/_Project/Resources/UI/"`)
   - Line 25: `"repo-alpha/Assets/_Project/Scripts/Systems/UI/<subsystem>/"` -> `"Assets/_Project/Scripts/Systems/UI/<subsystem>/"`
   - Line 26: `"repo-alpha/Assets/_Project/Resources/UI/Theme.uss"` -> `"Assets/_Project/Resources/UI/Theme.uss"`
   - Line 101: `"repo-alpha/Assets/_Project/Resources/UI/Theme.uss"` -> `"Assets/_Project/Resources/UI/Theme.uss"` (one occurrence — the "add it to" path; the "Also update" reference to mockup-builder does not contain repo-alpha)
   - Line 131: `"## repo-alpha-Specific Rules"` -> `"## Unity 6 Rules"` (the rules are Unity 6 specific, not repo-alpha specific — FontManager, StyleSheet loading, ScrollView height bug, etc.)

4. Edit `skills/debugging/implementer-prompt.md` — 1 replacement:
   - Line 39: `"repo-alpha.Tests.EditMode"` -> `"MyProject.Tests.EditMode"` (this is inside a placeholder comment showing example conventions, so a generic project name is appropriate)

5. Verification: After all edits, grep for "repo-alpha" (case-insensitive) across `skills/` directory. Expected: zero hits. (Hits in `docs/plans/` are expected and acceptable — historical documents are not updated.)

**Commit:** `refactor: de-repo-alpha mockup-builder, mock-to-unity, and debugging skills`
...
3. Add a **## Project Origin** section after the "## Origin" section (currently at line 107). Content:
   ...
   Crucible was developed alongside [repo-alpha](https://github.com/raddue/repo-alpha), a Unity 6 roguelike. Several skills reflect that Unity development context:
...
- Task 3: De-repo-alpha skills (4 modified files)
...
3. **Zero repo-alpha hits in skills/:**
   - `grep -ri "repo-alpha" skills/` returns zero results
   - `grep -ri "repo-alpha" docs/plans/` is expected to have hits (historical, acceptable)
